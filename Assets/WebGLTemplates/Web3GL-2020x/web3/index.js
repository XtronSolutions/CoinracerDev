...
async function ContractHash(p,a,k) {
    try {
        const pidInt = parseInt(p);
        const hash = web3.utils.keccak256(web3.eth.abi.encodeParameters(['uint256', 'address', 'string'], [pidInt, a, k]));
        window.web3gl.EncodedResponse = hash;
    } catch (error) {
        window.web3gl.EncodedResponse = error.message;
    }
}
