...
public class RegionManager : MonoBehaviour
{
    ...
    private string GetURL = "https://region.coinracer.io/"; //http://localhost:8008/ //http://64.227.31.248:8008/
    ...
    public void UpdateRegionsData(Dropdown dropdown, SetManualRegion _RegionRef)
    {
        ...
        if (Constants.StoredPings.Count > 0)
        {
            ...
            foreach (var item in Constants.RegionData)
            {
                Debug.Log(item.Key + " " + item.Value.address);
            }
            //dropdown.value = 1;
            for (int i = 0; i < Constants.StoredRegions.Count; i++)
            {
                Debug.Log("Selected copde: " + Constants.StoredRegions[i]);
