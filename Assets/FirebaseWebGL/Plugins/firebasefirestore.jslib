mergeInto(LibraryManager.library, {
    ...
    SetDocument: function (collectionPath, documentId, value, objectName, callback, fallback) {
		...
		var ref = db.collection(parsedPath).doc(parsedId);
        
		try {

            ref.set(JSON.parse(parsedValue)).then(function() {
				ref.update({ ProfileCreated: firebase.firestore.FieldValue.serverTimestamp() });
	...
	GetServerTimeStamp: function (objectName, callback, fallback) {
		var parsedObjectName = Pointer_stringify(objectName);
        var parsedCallback = Pointer_stringify(callback);
        var parsedFallback = Pointer_stringify(fallback);

        try {
			var _data=firebase.firestore.FieldValue.serverTimestamp();
			unityInstance.Module.SendMessage(parsedObjectName, parsedCallback, JSON.stringify(_data));						
        } catch (error) {
            unityInstance.Module.SendMessage(parsedObjectName, parsedFallback, JSON.stringify(error, Object.getOwnPropertyNames(error)));
        }
    },
